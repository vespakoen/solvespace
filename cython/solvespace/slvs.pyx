...
cdef class SolverSystem:
    ...
    def solve(self):
        print("solving...")
        cdef size_t ps = self.param_list.size()
        for i in range(ps):
            print("param", self.param_list[i])
        cdef size_t es = self.entity_list.size()
        for i in range(es):
            print("entity", self.entity_list[i])
        cdef size_t cs = self.cons_list.size()
        for i in range(cs):
            print("constraint", self.cons_list[i])
