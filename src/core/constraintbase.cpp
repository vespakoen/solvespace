...
bool ConstraintBase::IsProjectible() const {
    ...
    case Type::EQUAL_RADIUS: return false;
    }
...
Expr *ConstraintBase::PointLineDistance(hEntity wrkpl, hEntity hpt, hEntity hln) {
    ...
    if(wrkpl == EntityBase::FREE_IN_3D) {
        ...
        Expr *proj = (dv->Times(ua->Minus(u)))->Minus((du->Times(va->Minus(v))));
...
//-----------------------------------------------------------------------------
Expr *ConstraintBase::DirectionCosine(hEntity wrkpl, ExprVector ae, ExprVector be) {
    if(wrkpl == EntityBase::FREE_IN_3D) {
...
ExprVector ConstraintBase::PointInThreeSpace(hEntity workplane, Expr *u, Expr *v) {
...
Vector ConstraintBase::ProjectVectorInto(Vector a, EntityBase* w) const {
    // EntityBase *w = SK.GetEntity(wrkpl);
...
Vector ConstraintBase::ProjectInto(Vector a, EntityBase *w) const {
    // EntityBase *w = SK.GetEntity(wrkpl);
    Vector p0     = w->WorkplaneGetOffset();

    Vector f = a.Minus(p0);

    return p0.Plus(ProjectVectorInto(f, w));
}
...
void ConstraintBase::ModifyToSatisfy() {
    if(type == Type::ANGLE) {
        ...
        if(workplane != EntityBase::FREE_IN_3D) {
            EntityBase *w = SK.GetEntity(workplane);
            ProjectVectorInto(a, w);
            b = ProjectVectorInto(b, w);
        }
...
void ConstraintBase::GenerateEquations(IdList<Equation, hEquation> *l, bool forReference) const {
    if(reference && !forReference)
        return;

    Expr *exA = Expr::From(valA);
    switch(type) {
    case Type::PT_PT_DISTANCE: AddEq(l, Distance(workplane, ptA, ptB)->Minus(exA), 0); return;

    case Type::PROJ_PT_DISTANCE: {
        ExprVector pA = SK.GetEntity(ptA)->PointGetExprs(), pB = SK.GetEntity(ptB)->PointGetExprs(),
                   dp = pB.Minus(pA);
    ...
    case Type::EQUAL_LENGTH_LINES: {
        ...
        AddEq(l,
              Distance(workplane, a->point[0], a->point[1])
                  ->Minus(Distance(workplane, b->point[0], b->point[1])),
              0);
        return;
    }
    ...
    case Type::ARC_ARC_LEN_RATIO: {
        EntityBase *arc1 = SK.GetEntity(entityA), *arc2 = SK.GetEntity(entityB);

        // And get the arc1 radius, and the cosine of its angle
        EntityBase *ao1 = SK.GetEntity(arc1->point[0]), *as1 = SK.GetEntity(arc1->point[1]),
        ...
        EntityBase *ao2 = SK.GetEntity(arc2->point[0]), *as2 = SK.GetEntity(arc2->point[1]),
    ...
    case Type::ARC_LINE_LEN_RATIO: {
        EntityBase *line = SK.GetEntity(entityA), *arc1 = SK.GetEntity(entityB);
    ...
    case Type::EQUAL_LINE_ARC_LEN: {
        EntityBase *line = SK.GetEntity(entityA), *arc = SK.GetEntity(entityB);
        ...
        EntityBase *ao = SK.GetEntity(arc->point[0]), *as = SK.GetEntity(arc->point[1]),
    ...
    case Type::PT_ON_CIRCLE: {
        ...
        ExprVector u = normal->NormalExprsU(), v = normal->NormalExprsV();

        Expr *du = (center.Minus(pt)).Dot(u), *dv = (center.Minus(pt)).Dot(v);
    ...
    case Type::SYMMETRIC_LINE: {
        ...
        Expr *dista = (dlv->Times(lau->Minus(pau)))->Minus((dlu->Times(lav->Minus(pav))));
        Expr *distb = (dlv->Times(lau->Minus(pbu)))->Minus((dlu->Times(lav->Minus(pbv))));
    ...
    case Type::SAME_ORIENTATION: {
        ...
        ExprVector au = a->NormalExprsU(), an = a->NormalExprsN();
        ExprVector bu = b->NormalExprsU(), bv = b->NormalExprsV(), bn = b->NormalExprsN();
    ...
    case Type::ANGLE: {
        ...
        if(type == Type::ANGLE) {
            ...
            // specified angle
            Expr *rads = exA->Times(Expr::From(PI / 180)), *rc = rads->Cos();
