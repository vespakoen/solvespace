...
Quaternion EntityBase::NormalGetNum() const {
    Quaternion q;
    switch(type) {
    case Type::NORMAL_IN_3D: q = QuaternionFromParams(param[0], param[1], param[2], param[3]); break;
    ...
    case Type::NORMAL_N_ROT:
        q = QuaternionFromParams(param[0], param[1], param[2], param[3]);
        q = q.Times(numNormal);
        break;
...
void EntityBase::PointForceTo(Vector p) {
    ...
    case Type::POINT_N_ROT_AA: {
        ...
        Vector offset = VectorFromParams(param[0], param[1], param[2]);
        Vector normal = VectorFromParams(param[4], param[5], param[6]);
    ...
    case Type::POINT_N_ROT_AXIS_TRANS: {
        ...
        Vector offset = VectorFromParams(param[0], param[1], param[2]);
        Vector normal = VectorFromParams(param[4], param[5], param[6]).WithMagnitude(1.0);
...
Vector EntityBase::PointGetNum() const {
    Vector p;
    switch(type) {
    case Type::POINT_IN_3D: p = VectorFromParams(param[0], param[1], param[2]); break;
    ...
    case Type::POINT_N_TRANS: {
        Vector trans = VectorFromParams(param[0], param[1], param[2]);
    ...
    case Type::POINT_N_ROT_AXIS_TRANS: {
        Vector offset   = VectorFromParams(param[0], param[1], param[2]);
        Vector displace = VectorFromParams(param[4], param[5], param[6])
...
Quaternion EntityBase::QuaternionFromParams(hParam w, hParam vx, hParam vy, hParam vz) const {
    Quaternion q;
...
Quaternion EntityBase::PointGetQuaternion() const {
    ...
    if(type == Type::POINT_N_ROT_AA || type == Type::POINT_N_ROT_AXIS_TRANS) {
        ...
        q = QuaternionFromParams(param[3], param[4], param[5], param[6]);
    } else
        ssassert(false, "Unexpected entity type");
...
Vector EntityBase::FaceGetNormalNum() const {
    ...
    if(type == Type::FACE_NORMAL_PT) {
        ...
        Vector vc = VectorFromParams(param[0], param[1], param[2]);
        Vector vn = Vector::From(numNormal.vx, numNormal.vy, numNormal.vz);
        r         = vc.Cross(vn);
    } else if(type == Type::FACE_N_ROT_TRANS) {
        // The numerical normal vector gets the rotation
        r            = Vector::From(numNormal.vx, numNormal.vy, numNormal.vz);
        Quaternion q = QuaternionFromParams(param[3], param[4], param[5], param[6]);
        r            = q.Rotate(r);
    } else if(type == Type::FACE_N_TRANS) {
        r = Vector::From(numNormal.vx, numNormal.vy, numNormal.vz);
    } else if((type == Type::FACE_N_ROT_AA) || (type == Type::FACE_ROT_NORMAL_PT)) {
    ...
    case Type::ARC_OF_CIRCLE: {
        ...
        auto it = std::find_if(
            SK.constraint.begin(), SK.constraint.end(), [&](ConstraintBase const &con) {
                return (con.group == group) && (con.type == ConstraintBase::Type::POINTS_COINCIDENT) &&
